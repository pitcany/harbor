...
**Mounted vector-store patch (`chroma_patched.py`).** Unlike the idempotent `start_webui.sh` edits above, this is a read-only volume mount in `compose.webui.yml` that replaces WebUI's ChromaDB adapter (`…/retrieval/vector/dbs/chroma.py`) with `services/webui/chroma_patched.py`. Two changes vs stock: (1) collections are created with `hnsw:space: cosine` instead of WebUI's default L2 — the correct metric for normalized sentence-embedding vectors, which materially improves retrieval ranking; (2) the returned `distances` are remapped from Chroma cosine distance (`0`=best … `2`=worst) to a normalized similarity in `[0,1]` (`1`=best). This is load-bearing: all RAG retrieval (the `llama-*` native-RAG presets and the Math Proofs shim) depends on it. **It is a full-file override forked from 0.9.6 — re-verify it against upstream whenever you bump `HARBOR_WEBUI_VERSION`**, since an upstream adapter change would be silently masked by this older copy.
